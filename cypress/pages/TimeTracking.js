class TimeTracking {
  ...
  getIssueModal() {
    return cy.get(this.issueModal, { timeout: 60000 });
  }

  getIssueDetailModal() {
    return cy.get(this.issueDetailModal, {timeout: 60000 });
  }
  ...
  ensureTimeWasRemoved(title) {
    cy.reload();
    cy.get(this.backlogList, { timeout: 60000 }).contains(title).click({ force: true });
    cy.get(this.issueDetailModal).within(() => {
      cy.contains(this.noTime).should("exist");
    });
  }
